...
class KITTIDC(BaseDataset):
    ...
    def __getitem__(self, idx):
        rgb, depth, depth_MD, gt, K, num_sds = self._load_data(idx)
        ...
        if self.augment and self.mode == 'train':
            ...
            if self.args.top_crop > 0:
                ...
                depth = TF.crop(depth, self.args.top_crop, 0,
                                height - self.args.top_crop, width)
                depth_MD = TF.crop(depth_MD, self.args.top_crop, 0,
                                   height - self.args.top_crop, width)
            ...
            depth_MD = TF.rotate(depth_MD, angle=degree, interpolation=InterpolationMode.NEAREST)
            ...
            depth_MD = TF.resize(depth_MD, scale, interpolation=InterpolationMode.NEAREST)
            ...
            depth_MD = TF.crop(depth_MD, h_start, w_start, ch, cw)
            ...
            depth_MD = TF.to_tensor(np.array(depth_MD))
            depth_MD = depth_MD / _scale
            ...
            depth_MD = TF.crop(depth_MD, h_start, w_start, self.height, self.width)
        ...
        output = {'rgb': rgb, 'dep': depth, 'dep_MD': depth_MD, 'mask_init': mask_init, 'gt': gt, 'K': torch.Tensor(K), 'num_sample': num_sds}
    ...
    def _load_data(self, idx):
        ...
        if self.mode == 'train':
            path_depth_MD = os.path.join(self.args.dir_data,
                                         self.sample_list[idx]['depth']).replace('data_depth_velodyne', 'data_depth_velodyne_NewCRFs')
        else:
            path_depth_MD = os.path.join(self.args.dir_data,
                                         self.sample_list[idx]['depth']).replace('/velodyne_raw/', '/velodyne_NewCRFs/')

        depth = read_depth(path_depth)
        gt = read_depth(path_gt)
        depth_MD = read_depth(path_depth_MD)
        ...
        if self.mode == 'train':
            if self.args.train_with_random_sds:
                num_lines = random.randint(self.args.lidar_lines_train_from, self.args.lidar_lines_train_to)
            else:
                num_lines = self.args.lidar_lines_train_to
        else:
            num_lines = self.args.lidar_lines_test

        num_depths = (np.array(depth) > 0).sum()

        keep_ratio = (num_lines / 64.0)
        ...
        if keep_ratio >= 0.9999:
            ...
            Km[0, 2] = K[2]
            Km[1, 2] = K[3]
            depth = sample_lidar_lines(self.args, self.mode, depth[:, :, None], intrinsics=Km, num_lines=num_lines, keep_ratio=keep_ratio)[:, :, 0]
        else:
            depth = np.zeros_like(depth)

        rgb = Image.open(path_rgb)
        depth = Image.fromarray(depth.astype('float32'), mode='F')
        depth_MD = Image.fromarray(depth_MD.astype('float32'), mode='F')
        ...
        num_sds = int(num_depths * keep_ratio)

        return rgb, depth, depth_MD, gt, K, num_sds
