...
class BaseSummary(SummaryWriter):
    def __init__(self, log_dir, mode, args):
        ...
        f_tmp = open(self.f_loss, 'a')
        f_tmp.close()
        f_tmp = open(self.f_metric, 'a')
        f_tmp.close()

        self.f_loss_csv = '{}/csv/loss_{}.csv'.format(log_dir, mode)
        self.f_metric_csv = '{}/csv/metric_{}.csv'.format(log_dir, mode)
