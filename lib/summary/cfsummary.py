...
class CompletionFormerSummary(BaseSummary):
    ...
    def update(self, global_step, sample, output):
        if self.loss_name is not None:
            ...
            f_loss.close()
            write_csv(self.args, self.f_loss_csv, global_step, self.loss_name, self.loss)
        ...
        if self.metric_name is not None:
            ...
            f_metric.write('{:04d} |{}\n'.format(global_step, msg))
            f_metric.close()
            write_csv(self.args, self.f_metric_csv, global_step, self.metric_name, self.metric)
        ...
        for b in range(0, num_summary):
            ...
            img = np.concatenate((rgb_tmp, dep_tmp, pred_tmp, gt_tmp, error_tmp), axis=1)
    ...
    def save(self, epoch, idx, sample, output):
        with torch.no_grad():
            if self.args.save_result_npy:
                self.path_output = '{}/{}/epoch{:04d}'.format(self.log_dir,
                                                              self.mode, epoch)
                os.makedirs(self.path_output, exist_ok=True)

                path_save_rgb = '{}/{:010d}_rgb.png'.format(self.path_output, idx)
                path_save_pred = '{}/{:010d}_pred.npy'.format(self.path_output, idx)
                path_save_dep = '{}/{:010d}_dep.npy'.format(self.path_output, idx)
                path_save_gt = '{}/{:010d}_gt.npy'.format(self.path_output, idx)
                ...
                np.save(path_save_rgb, rgb)
                np.save(path_save_pred, pred)
                np.save(path_save_dep, dep)
                np.save(path_save_gt, gt)

                for iter in range(len(output['pred_inter'])):
                    pred_inter = output['pred_inter'][iter].detach()
                    mask_inter = output['mask_inter'][iter].detach()

                    pred_idx = pred_inter[0, 0, :, :].data.cpu().numpy()
                    mask_idx = mask_inter[0, 0, :, :].data.cpu().numpy()

                    path_save_pred_inter = '{}/{:010d}_pred_inter_{:02d}.npy'.format(self.path_output, idx, iter)
                    path_save_mask_inter = '{}/{:010d}_mask_inter_{:02d}.npy'.format(self.path_output, idx, iter)

                    np.save(path_save_pred_inter, pred_idx)
                    np.save(path_save_mask_inter, mask_idx)
            ...
            if self.args.save_result_only:
                ...
                for k in range(0, len(list_feat)):
                    ...
                    feat_inter = np.concatenate((rgb, cm(norm(feat_inter))[...,:3], cm(norm(gt))[...,:3], depth_err_to_colorbar(feat_inter, gt)), axis=1)
                ...
                if 'mask_inter' in output:
                    list_mask = output['mask_inter']
                    for k in range(0, len(list_mask)):
                        mask_inter = list_mask[k]
                        mask_inter = mask_inter[0, 0, :, :].data.cpu().numpy()

                        path_save_mask = '{}/04_pred_mask_{:02d}.png'.format(
                            self.path_output, k)
                        plt.imsave(path_save_mask, mask_inter)
